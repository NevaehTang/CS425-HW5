...
dump_name = 'dump-' + str(nframes) + '.dat'

flagMakeNewData = input('Make new game data(' + dump_name + ')? [0 or 1]') == '1'
flagFit = True
...
if (flagMakeNewData):
    ...
    for episode in range(100):
        ...
        if (episode == 1):
            with open(dump_path + dump_name, 'wb') as dump_out:
        ...
        for step in range(10000 - nframes):
            ...
            if (n_frames_reward > 0.):
                with open(dump_path + dump_name, 'ab') as dump_out:
...
if (flagFit):
    ...
    print('Start fit')
    with open(dump_path + dump_name, 'rb') as dump_in:
        counter = 100000
